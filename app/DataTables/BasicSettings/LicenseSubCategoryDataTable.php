...
class LicenseSubCategoryDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'backend.pages.license.subcategory.action')
     ...
     */
    public function query(LicenseSubCategory $model): QueryBuilder
    {
        $category_id = $this->category_id ?? $this->request()->route('category_id');
        $query = $model->newQuery()->with('category');
        if ($category_id) {
            $query->where('license_category_id', $category_id);
        }
        return $query;
    }
    ...
    {
        return $this->builder()
            ->setTableId('licensesubcategory-table')
            ...
            ->parameters([
                ...
                'language' => [
                    ...
                    'infoEmpty' => 'কোনo রেকর্ড উপলব্ধ নেই',
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('category')->title('License Category')->orderable(false)->searchable(false),
    ...
    {
        return 'LicenseSubCategory_' . date('YmdHis');
    }
}
