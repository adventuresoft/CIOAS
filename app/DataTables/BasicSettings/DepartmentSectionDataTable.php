...
class DepartmentSectionDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) {
                $showButton = '<a class="btn btn-sm btn-info mr-1" title="Show" data-toggle="tooltip" href="' . route('basic-settings.department-section.show', $row->id) . '"><i class="fa fa-eye"></i></a>';
                $editButton = '<a class="btn btn-sm btn-primary mr-1" title="Edit" data-toggle="tooltip" href="' . route('basic-settings.department-section.edit', $row->id) . '"><i class="fa fa-edit"></i></a>';
                $deleteForm = '<form class="deleteData" method="post" style="display:inline-block; margin:0;">                          
                                    ...
                                    <input type="hidden" class="deleteUrl" name="deleteUrl" value="' . route('basic-settings.department-section.destroy', $row->id) . '">
                                    <input type="hidden" class="redirect-url" name="redirectUrl" value="' . route('basic-settings.department-section.index', $row->department_id) . '">
                                    <button type="submit" title="Delete" data-toggle="tooltip" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></button>
                               </form>';
                return '<div class="table-action" style="display:flex; align-items:center;">' . $showButton . $editButton . $deleteForm . '</div>';
            })
            ->rawColumns(['action'])
            ->setRowId('id');
    }
    ...
    {
        $department_id = $this->request()->route('department_id');
    ...
    {
        return $this->builder()
            ->setTableId('department-section-table')
    ...
    {
        return 'DepartmentSection_' . date('YmdHis');
    }
}
