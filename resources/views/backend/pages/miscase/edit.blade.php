...
@php
    ...
    $plaintiffs = old('plaintiffs', $miscase->plaintiffs ?? []);
    $defendants = old('defendants', $miscase->defendants ?? []);
    ...
    // If no old input, try to read saved location rows from model notes or legacy land_info
    if (empty($savedLocationRows)) {
        if (!empty($miscase->notes)) {
            $notesArr = is_string($miscase->notes) ? json_decode($miscase->notes, true) : (array) $miscase->notes;
            $savedLocationRows = $notesArr['location_rows'] ?? ($miscase->land_info ?? []);
        } else {
            $savedLocationRows = $miscase->land_info ?? [];
        }
    }
    $locationRows = [];
    ...
    <div class="container-fluid">
        <div class="miscase-shell">
            ...
            <form id="FormSubmit" method="POST" enctype="multipart/form-data" action="{{ $formAction }}"
                            ...
                            <div class="col-md-4">
                                <div class="md-field">
                                    ...
                                    <input type="date" name="case_date" id="case_date" class="form-control md-control"
                                        value="{{ old('case_date', optional($miscase->case_date ?? null)->format('Y-m-d') ?? date('Y-m-d')) }}"
                                        required>
                            ...
                            <div class="col-md-4">
                                <div class="md-field">
                                    ...
                                    <select name="case_type" id="case_type" class="form-control ">
                                        ...
                                        <option value="1" @selected(old('case_type', $miscase->case_type ?? '') == '1')>শ্রেণি পরিবর্তন</option>
                                        <option value="2" @selected(old('case_type', $miscase->case_type ?? '') == '2')> নামজারি সংক্রান্ত</option>
                                        <option value="3" @selected(old('case_type', $miscase->case_type ?? '') == '3')>মুদ্রণজনিত ত্রুটি</option>
                                        <option value="4" @selected(old('case_type', $miscase->case_type ?? '') == '4')>নিলাম বিজ্ঞপ্তি</option>
                                        <option value="5" @selected(old('case_type', $miscase->case_type ?? '') == '5')>অর্পিত থেকে খাস করা</option>
                                        <option value="6" @selected(old('case_type', $miscase->case_type ?? '') == '6')>বন্দোবস্ত মামলা</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="md-field">
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text" style="border: 1px solid var(--mc-line); border-radius: 8px 0 0 8px; background: #f8fafc; font-weight: bold; color: var(--mc-primary); display: flex; align-items: center; justify-content: center; min-width: 42px; min-height: 42px;">৳</span>
                                        </div>
                                        <input type="number" step="0.01" name="case_fee" id="case_fee"
                                            class="form-control md-control" style="border-radius: 0 8px 8px 0 !important; border-left: none !important;"
                                            value="{{ old('case_fee', $miscase->case_fee ?? '') }}" placeholder="0.00">
                                    </div>
                            ...
                            <div class="col-md-4">
                                <div class="md-field">
                                    ...
                                    <input type="text" name="case_reason" id="case_reason"
                                        ...
                                        value="{{ old('case_reason', $miscase->case_reason ?? '') }}"
                            ...
                            <div class="col-md-4">
                                <div class="md-field">
                                    ...
                                    <select name="status" id="status" class="form-control">
                                        <option value="draft" @selected(old('status', $miscase->status ?? '') == 'draft')>Draft</option>
                                        <option value="running" @selected(old('status', $miscase->status ?? '') == 'running')>Running</option>
                                        <option value="closed" @selected(old('status', $miscase->status ?? '') == 'closed')>Closed</option>
                            ...
                            <div class="col-md-12">
                                <div class="md-field">
                                    ...
                                    <textarea name="case_details" id="case_details" class="form-control md-control"
                                        rows="4"
                                        placeholder="Write case summary">{{ old('case_details', $miscase->case_details ?? '') }}</textarea>
                            ...
                            <div class="col-md-12">
                                <div class="md-field mb-0">
                                    ...
                                    <textarea name="rejection_reason" id="rejection_reason"
                                        ...
                                        placeholder="Required only when case is rejected">{{ old('rejection_reason', $miscase->rejection_reason ?? '') }}</textarea>
                ...
                <div class="miscase-panel">
                    <div class="miscase-panel-header">
                        <h3 class="miscase-panel-title"><i class="fas fa-map-marked-alt"></i> Location And Land Records</h3>
                                    ...
                                    @if (!empty($miscase->files))
                                        <div class="mt-3" style="display:flex; flex-direction:column; gap:8px;">
                                            @foreach ($miscase->files as $file)
                                                ...
                                                @endphp
                                                @if ($filePath)
                                                    <div style="display:flex; align-items:center; gap:8px; background:#f1f5f9; padding:8px 12px; border-radius:6px; border:1px solid #cbd5e1;">
                                                        <i class="fas fa-file-alt text-teal" style="font-size:16px; color:#0f766e;"></i>
                                                        <a href="{{ asset($filePath) }}" target="_blank"
                                                            style="font-size:13px; font-weight:600; color:#1e293b; text-decoration:none; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:180px;">
                                                            {{ $fileName }}
                                                        </a>
                                                    </div>
                                                @endif
