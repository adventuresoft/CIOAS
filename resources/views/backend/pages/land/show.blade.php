...
@section('content')
    <x-print-view title="সরকারি স্বার্থ সংশ্লিষ্ট জমির বিবরণ">
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 25px; font-size: 15px; font-weight: 600; color: #1e293b;">
            <div>
                <div style="margin-bottom: 5px;">জমির আইডি: <span style="font-weight: normal;">{{ str_pad($land->id, 7, '0', STR_PAD_LEFT) }}-{{ date('m-Y') }}</span></div>
                <div>জমির ধরণ: <span style="font-weight: normal;">{{ $land->land_type }}</span></div>
            </div>
            <div></div>
        </div>

        @php
            $firstLoc = $land->locations ? $land->locations->first() : null;
        @endphp
        ...
        <div style="text-align: center; font-weight: 700; font-size: 15px; color: #0f766e; margin-bottom: 20px; border: 1px dashed #cbd5e1; padding: 10px; border-radius: 6px; background-color: #f8fafc;">
            সর্বশেষ রেকর্ড: {{ $firstLoc->record_type ?? '—' }}, 
            জেলা: {{ $firstLoc->district->name ?? '—' }}, 
            উপজেলা/সার্কেল: {{ $firstLoc->upazila->name ?? '—' }}, 
            মৌজা: {{ $firstLoc->mouza->name ?? '—' }}, 
            জেএল নং: {{ $firstLoc->mouza->jl_no ?? '—' }}
        </div>
            ...
            <tbody>
                @forelse($land->details as $row)
                    <tr>
                        ...
                        <td>{{ $row->recorded_class }}</td>
                        <td>{{ $row->actual_class }}</td>
                        <td>{{ number_format((float) $row->total_land, 4) }}</td>
                        <td style="font-weight: 600; color: #0f766e;">{{ number_format((float) $row->land_amount, 4) }}</td>
        ...
        <table class="data-table" style="margin-top: 30px;">
            <tr>
                <td style="width: 33.33%; vertical-align: top; padding: 15px;">
                    ...
                    @php
                        $hasCase = $land->details->whereNotNull('case_no')->where('case_no', '!=', '')->count() > 0 ? 'হ্যাঁ' : 'না';
                        $firstCase = $land->details->whereNotNull('case_no')->where('case_no', '!=', '')->first();
                    @endphp
                ...
                <td style="width: 33.33%; vertical-align: top; padding: 15px;">
                    ...
                    @php
                        $possession = optional($land->details->first())->possession_status ?? '';
                    @endphp
                    ...
                    @if(count($land->documents))
                        <ul style="margin: 0; padding-left: 20px; color: #1e293b;">
                            @foreach($land->documents as $doc)
                                <li style="margin-bottom: 5px;">{{ $doc->document_name }}</li>
                            @endforeach
                        </ul>
                    @else
        ...
        <style>
            @media print {
                .custom-actions { display: none !important; }
            }
        </style>
        
        <!-- Additional custom action buttons for this specific page -->
        <div class="text-center mt-4 mb-2 custom-actions d-print-none">
            <a href="{{ route('land.edit', $land->id) }}" class="btn btn-primary btn-sm px-4 ms-2">
                <i class="fas fa-edit"></i> সম্পাদনা করুন
            </a>
            <a href="{{ route('land.index') }}" class="btn btn-secondary btn-sm px-4 ms-2">
                <i class="fas fa-list"></i> তালিকায় ফিরুন
            </a>
        </div>
