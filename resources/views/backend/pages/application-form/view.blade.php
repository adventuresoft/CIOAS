...
@push('style')
    ...
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Noto+Sans+Bengali:wght@300;400;500;600;700;800&family=Courier+Prime:wght@400;700&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        .watermark-stamp.approved { color: #006a4e; border-color: #006a4e; }
        .watermark-stamp.processing { color: #d97706; border-color: #d97706; }
        .watermark-stamp.rejected { color: #dc2626; border-color: #dc2626; }
        .watermark-stamp.pending { color: #4b5563; border-color: #4b5563; }
        ...
        .timeline-node.success .timeline-icon { background-color: #16a34a !important; box-shadow: 0 0 0 3px rgba(22, 163, 74, 0.15); }
        .timeline-node.info .timeline-icon { background-color: #0284c7 !important; box-shadow: 0 0 0 3px rgba(2, 132, 199, 0.15); }
        .timeline-node.warning .timeline-icon { background-color: #d97706 !important; box-shadow: 0 0 0 3px rgba(217, 119, 6, 0.15); }
        .timeline-node.danger .timeline-icon { background-color: #dc2626 !important; box-shadow: 0 0 0 3px rgba(220, 38, 38, 0.15); }
        ...
        .capsule-badge.pending { background-color: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; }
        .capsule-badge.assigned { background-color: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd; }
        .capsule-badge.received { background-color: #f3e8ff; color: #6b21a8; border: 1px solid #e9d5ff; }
        .capsule-badge.processing { background-color: #fef9c3; color: #854d0e; border: 1px solid #fef08a; }
        .capsule-badge.approved { background-color: #dcfce7; color: #166534; border: 1px solid #bbf7d0; }
        .capsule-badge.rejected { background-color: #fee2e2; color: #991b1b; border: 1px solid #fecaca; }
        ...
        @media print {
            ...
            .main-sidebar, .main-header, .main-footer, .no-print, .breadcrumb, .content-header, .btn, .control-card {
    ...
    <section class="content-header no-print">
        <div class="container-fluid">
            <div class="row mb-2 align-items-center">
                <div class="col-sm-6">
                    <h1 class="font-weight-bold text-dark" style="font-size: 24px;"><i class="fa fa-file-alt text-info"></i> Application Form Details</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content application-view-container">
        <div class="container-fluid">
            ...
            @if($applicationForm->revision_note && $applicationForm->final_approved_by ==  null)
                <div class="alert alert-warning no-print border-0 p-3 mb-3 d-flex align-items-start shadow-sm" style="border-left: 4px solid #d97706 !important; background-color: #fffbeb; border-radius: 6px;">
                    <i class="fa fa-exclamation-triangle mt-1 mr-3 text-warning" style="font-size: 18px;"></i>
                    <div>
                        <h6 class="font-weight-bold text-warning-deep mb-1" style="color: #b45309;"><i class="fa fa-edit"></i> সংশোধন নির্দেশনা (Revision Instruction)</h6>
                        <p class="mb-0 text-dark" style="white-space: pre-line; font-size:13.5px;">{{ $applicationForm->revision_note }}</p>
                    </div>
                </div>
            @endif
            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-12 print-document-area">
                    <div class="document-paper">
                        ...
                        <div class="govt-header text-center position-relative">
                            <div class="row align-items-center">
                                <div class="col-3 text-left govt-logo-container">
                                    <img src="{{ asset('images/govt-bd-logo.png') }}" alt="Government Seal" onerror="this.src='https://upload.wikimedia.org/wikipedia/commons/8/84/Government_Seal_of_Bangladesh.svg'; this.onerror=null;" style="height: 75px; width: auto; filter: drop-shadow(0px 2px 3px rgba(0,0,0,0.08));">
                                </div>
                                <div class="col-6 text-center">
                                    ...
                                    <p class="govt-address mb-0">
                                        উপজেলা: {{ Auth::user()->institute->union->thana->bn_name ?? 'সদর' }}, জেলা: {{ Auth::user()->institute->union->thana->district->bn_name ?? 'ঢাকা' }}, বাংলাদেশ।
                                    </p>
                                </div>
                                <div class="col-3 text-right govt-logo-container">
                                    <img src="{{ asset('images/dhaka.png') }}" alt="Local Logo" onerror="this.style.display='none';" style="height: 75px; width: auto;">
                                </div>
                            </div>
                            <div class="govt-divider" style="height: 3px; background: linear-gradient(90deg, #006a4e 0%, #006a4e 80%, #f42a41 80%, #f42a41 100%); margin-top: 15px;"></div>
                        </div>

                        <!-- Memorandum and Date Grid -->
                        <div class="row memo-info align-items-center">
                            <div class="col-6">
                                <strong>স্মারক নম্বর:</strong> <span class="mono-num" style="color: #006a4e; font-size:14.5px;">{{ $applicationForm->application_number ?? '-' }}</span>
                            </div>
                            <div class="col-6 text-right">
                                <strong>তারিখ:</strong> <span class="mono-num">{{ $applicationForm->date ? date('d/m/Y', strtotime($applicationForm->date)) : '-' }} খ্রিঃ</span>
                            </div>
                        </div>
                        ...
                        <div class="letter-salutation">
                            বরাবর,<br>
                            <span class="font-weight-bold text-dark">{{ $applicationForm->recipient ?? 'দায়িত্বপ্রাপ্ত কর্মকর্তা' }}</span>
                        </div>
                        ...
                        <div class="row mt-5 pt-4" style="border-top: 1px solid #f1f5f9;">
                            <div class="col-md-6 mb-4">
                                <h6 style="color:#006a4e; font-weight:700; font-size:14.5px; margin-bottom:12px;"><i class="fa fa-user-circle"></i> আবেদনকারীর বিবরণ:</h6>
                                <table class="table table-sm table-borderless" style="font-size: 13.5px; line-height: 1.6; margin-bottom: 0;">
                                    <tr>
                                        <td style="width: 100px; color:#64748b; padding: 2px 0;">প্রেরক (Sender):</td>
                                        <td style="font-weight:700; color: #1e293b; padding: 2px 0;">{{ $applicationForm->sender ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#64748b; padding: 2px 0;">মোবাইল নম্বর:</td>
                                        <td style="font-weight:700; color: #1e293b; padding: 2px 0;">{{ $applicationForm->mobile ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#64748b; padding: 2px 0;">এনআইডি নম্বর:</td>
                                        <td style="font-weight:700; color: #1e293b; padding: 2px 0;">{{ $applicationForm->nid_no ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#64748b; padding: 2px 0;">ঠিকানা:</td>
                                        <td style="font-weight:700; color: #1e293b; padding: 2px 0; white-space: pre-line;">{{ $applicationForm->address ?: '-' }}</td>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-md-6 mb-4">
                                <h6 style="color:#006a4e; font-weight:700; font-size:14.5px; margin-bottom:12px;"><i class="fa fa-info-circle"></i> আবেদনের দাপ্তরিক বিবরণ:</h6>
                                <table class="table table-sm table-borderless" style="font-size: 13.5px; line-height: 1.6; margin-bottom: 0;">
                                    <tr>
                                        <td style="width: 120px; color:#64748b; padding: 2px 0;">আবেদনের ধরণ:</td>
                                        <td style="font-weight:700; color: #1e293b; padding: 2px 0;">{{ $applicationForm->form_type ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#64748b; padding: 2px 0;">দাখিলকারী (Inward):</td>
                                        <td style="font-weight:700; color: #1e293b; padding: 2px 0;">{{ $applicationForm->receiver->name ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#64748b; padding: 2px 0;">দাখিলের সময়:</td>
                                        <td style="font-weight:700; color: #1e293b; padding: 2px 0;">{{ $applicationForm->created_at ? $applicationForm->created_at->format('d M, Y h:i A') : '-' }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                        ...
                        @if ($applicationForm->attachment)
                            <div class="mt-4 pt-3 no-print" style="border-top: 1px solid #f1f5f9;">
                                <h6 style="color:#006a4e; font-weight:700; font-size:14.5px; margin-bottom:12px;"><i class="fa fa-paperclip"></i> আবেদনপত্রের সংযুক্তি (Attachment):</h6>
                                <a href="{{ asset($applicationForm->attachment) }}" target="_blank" class="attachment-badge">
                                    ...
                                    <i class="fa fa-external-link-alt" style="font-size:10px; margin-left:5px; opacity:0.6;"></i>
                                </a>
                            </div>
                        @endif
                        ...
                        <div class="row signature-row mt-5">
                            <div class="col-md-6 mb-4">
                                <div class="sig-block">
                                    <span class="sig-stamp-placeholder warn"><i class="fa fa-check-circle"></i> প্রথম অনুমোদন</span>
                                    <div style="height: 45px; display: flex; align-items: center; justify-content: center;">
                                        @if($applicationForm->initialApprover || $applicationForm->approver)
                                            <div style="font-family:'Courier Prime', monospace; color: #16a34a; font-weight: 700; font-size:15px; border: 2px solid #16a34a; padding: 2px 8px; border-radius: 4px; transform: rotate(-3deg);">
                                                ✓ INITIAL OK
                                            </div>
                                        @else
                                            <span style="color:#cbd5e1; font-size: 13px; font-style: italic;">অপেক্ষমান (Pending)</span>
                                        @endif
                                    </div>
                                    <div class="sig-name">{{ $applicationForm->initialApprover->name ?? ($applicationForm->approver->name ?? '-') }}</div>
                                    ...
                                    @if($applicationForm->initial_approval_note ?? $applicationForm->approval_note)
                                        <div style="font-size: 11.5px; background: #fffbeb; border: 1px solid #fef3c7; border-radius: 4px; padding: 6px; margin-top: 8px; text-align: left; color: #b45309; word-break: break-word; line-height: 1.4;">
                                            <strong>নোট:</strong> {{ $applicationForm->initial_approval_note ?? $applicationForm->approval_note }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-6 mb-4">
                                <div class="sig-block">
                                    <span class="sig-stamp-placeholder"><i class="fa fa-check-double"></i> চূড়ান্ত অনুমোদন</span>
                                    <div style="height: 45px; display: flex; align-items: center; justify-content: center;">
                                        @if($applicationForm->finalApprover)
                                            <div style="font-family:'Courier Prime', monospace; color: #16a34a; font-weight: 700; font-size:15px; border: 2px double #16a34a; padding: 2px 8px; border-radius: 4px; transform: rotate(-3deg);">
                                                ✓ FINAL APPROVED
                                            </div>
                                        @else
                                            <span style="color:#cbd5e1; font-size: 13px; font-style: italic;">অপেক্ষমান (Pending)</span>
                                    ...
                                    <div class="sig-date">{{ $applicationForm->final_approved_at ? $applicationForm->final_approved_at->format('d M, Y h:i A') : '-' }}</div>
                                    @if($applicationForm->final_approval_note)
                                        <div style="font-size: 11.5px; background: #ecfdf5; border: 1px solid #d1fae5; border-radius: 4px; padding: 6px; margin-top: 8px; text-align: left; color: #065f46; word-break: break-word; line-height: 1.4;">
                ...
                <div class="col-lg-4 col-md-12 no-print">
                    ...
                    <div class="control-card">
                        ...
                        <div class="control-card-body">
                            <div class="text-center mb-4 pb-3 border-bottom">
                                ...
                                <h6 class="text-muted font-weight-bold mb-0 mt-1" style="font-size: 13.5px;">আবেদন নং: #{{ $applicationForm->application_number ?? '-' }}</h6>
                            </div>
                            ...
                            <div class="meta-item">
                                ...
                                <div class="meta-val">{{ $applicationForm->initialApprover->name ?? ($applicationForm->receiver->name ?? '-') }}</div>
                            </div>

                            <div class="mt-4">
                                <a href="{{ route('application-form.index') }}" class="btn btn-sm btn-outline-dark w-100 font-weight-bold">
                    ...
                    @if ($applicationForm->status === 'approved')
                        ...
                        <div class="control-card">
                            <div class="control-card-header bg-success text-white" style="background-color: #166534 !important;">
                                <i class="fa fa-stamp"></i> সিদ্ধান্ত ও অনুমোদন প্যানেল
                            </div>
                            <div class="control-card-body">
                                <div class="form-group mb-0">
                                    <label style="font-size:12px; font-weight:600; color:#555;">অনুমোদন নোট (Approval Note)</label>
                                    <div class="p-3 bg-light border rounded text-dark mb-0" style="font-size: 13.5px; white-space: pre-wrap; line-height:1.5; border-radius: 6px;">
                                    ...
                                    <small class="text-success d-block mt-3 font-weight-bold" style="font-size: 13px;">
                                        <i class="fa fa-check-double"></i> আবেদনটি চূড়ান্তভাবে অনুমোদিত হয়েছে। (The application has been approved)
                                    </small>
                                </div>
                            </div>
                        </div>
                    @elseif ($applicationForm->status === 'rejected')
                        ...
                        <div class="control-card">
                            ...
                            <div class="control-card-body">
                                <div class="form-group mb-0">
                                    <label style="font-size:12px; font-weight:600; color:#555;">বাতিল মন্তব্য (Rejection Note)</label>
                                    <div class="p-3 bg-light border rounded text-dark mb-0" style="font-size: 13.5px; white-space: pre-wrap; line-height:1.5; border-radius: 6px;">
                                        {{ $applicationForm->approval_note ?: 'কোন মন্তব্য করা হয়নি।' }}
                                    </div>
                                    <small class="text-danger d-block mt-3 font-weight-bold" style="font-size: 13px;">
                                        <i class="fa fa-times-circle"></i> আবেদনটি বাতিল করা হয়েছে। (The application has been rejected)
                                    </small>
                                </div>
                            </div>
                        </div>
                    @else
                        ...
                        @if ($canAssign || $canReceive || ($showApproveForm ?? false))
                            <div class="control-card">
                                <div class="control-card-header bg-success text-white" style="background-color: #006a4e !important;">
                                ...
                                <div class="control-card-body">
                                    ...
                                    @if ($canReceive)
                                        <form id="receiveForm" method="POST" class="mb-3">
                                            ...
                                            <div class="alert alert-info p-2 mb-2" style="font-size:12.5px; border-radius: 4px;">
                                                <i class="fa fa-info-circle"></i> আবেদনটি আপনার দপ্তর বা শাখায় হস্তান্তরের জন্য প্রথমে <strong>রিসিভ (Receive)</strong> করুন।
                                            </div>
                                    ...
                                    @if ($canAssign)
                                        <form id="assignForm" method="POST" class="mb-3 border-top pt-3">
                                            @csrf
                                            <h6 class="font-weight-bold text-dark mb-2" style="font-size:13.5px;"><i class="fa fa-share"></i> নথি প্রেরণ / রি-অ্যাসাইন করুন:</h6>
                                            
                                            <div class="form-group mb-2">
                                                <label for="department_id" style="font-size:12px; font-weight:600; color:#555;">বিভাগ (Department)</label>
                                            ...
                                            <div class="form-group mb-2">
                                                <label for="section_id" style="font-size:12px; font-weight:600; color:#555;">শাখা (Section)</label>
                                            ...
                                            <div class="form-group mb-3">
                                                <label for="assign_note" style="font-size:12px; font-weight:600; color:#555;">মন্তব্য (Assignment Note)</label>
                                                <textarea name="note" id="assign_note" rows="2" class="form-control form-control-sm" placeholder="প্রেরণের কারণ বা নির্দেশনা লিখুন (ঐচ্ছিক)"></textarea>
                                    ...
                                    @if ($showApproveForm ?? false)
                                        <form id="approveForm" method="POST" class="border-top pt-3">
                                            ...
                                            <h6 class="font-weight-bold text-dark mb-2" style="font-size:13.5px;"><i class="fa fa-stamp"></i> সিদ্ধান্ত ও অনুমোদন প্যানেল:</h6>
                                            
                                            <div class="form-group mb-3">
                                                <label for="approval_note" style="font-size:12px; font-weight:600; color:#555;">সিদ্ধান্ত নোট (Note/Remarks) <span class="text-danger">*</span></label>
                                                <textarea name="approval_note" id="approval_note" rows="3" class="form-control form-control-sm"
                                                ...
                                                @if ($applicationForm->status === 'processing' && !$canApprove)
                                                    <small class="text-warning d-block mt-1"><i class="fa fa-info-circle"></i> আবেদনটি বর্তমানে প্রক্রিয়াধীন (Processing) আছে। চূড়ান্ত অনুমোদনের ক্ষমতা আপনার নেই।</small>
                                                @elseif (!$canApprove)
                                                    <small class="text-muted d-block mt-1"><i class="fa fa-exclamation-triangle"></i> ফাইলটি রিসিভ করার পর সিদ্ধান্ত প্রদান সচল হবে।</small>
                                                @endif
                                            </div>
                                            ...
                                            @if ($canApprove)
                                                <div class="row g-2">
                                                    @if ($applicationForm->status === 'processing')
                                                        <div class="col-4">
                                                            <button type="button" class="btn btn-warning btn-sm text-white w-100 font-weight-bold" id="revisionBtn">
                                                                <i class="fa fa-undo"></i> সংশোধন
                                                            </button>
                                                        </div>
                                                        <div class="col-4">
                                                            <button type="button" class="btn btn-danger btn-sm w-100 font-weight-bold" id="rejectBtn">
                                                                <i class="fa fa-times"></i> বাতিল
                                                            </button>
                                                        </div>
                                                        <div class="col-4">
                                                            <button type="submit" class="btn btn-success btn-sm w-100 font-weight-bold" id="approveBtn">
                                    ...
                                    <div class="timeline-node {{ $nodeClass }}">
                                        ...
                                        <div class="timeline-content">
                                            <div class="timeline-time">
                                                <i class="far fa-clock"></i> {{ $assign->created_at ? $assign->created_at->format('d M, Y h:i A') : '-' }}
                                            </div>
                                            ...
                                            <div class="timeline-desc">
                                                <strong>প্রাপক:</strong> {{ $assign->toDepartment->name ?? '-' }} / {{ $assign->toSection->name ?? '-' }}<br>
                                                <strong>প্রেরক:</strong> {{ $assign->fromDepartment->name ?? '-' }} / {{ $assign->fromSection->name ?? '-' }}<br>
                                                <strong>দ্বারা:</strong> {{ $assign->assignedByUser->name ?? '-' }}
                                                @if($assign->receivedByUser)
                                                    <br><strong>গ্রহীতা:</strong> {{ $assign->receivedByUser->name }} ({{ $assign->received_at ? $assign->received_at->format('d M, Y h:i A') : '-' }})
                                                @endif
                                            </div>
                                            @if($assign->note)
