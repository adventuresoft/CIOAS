        ...
        <div class="container-fluid">
            <div class="cioas-shell">
                <div class="d-flex align-items-center mb-4" style="gap: 12px;">
                    <a href="{{ route('license.edit', $organization->id) }}" class="btn btn-light border rounded-pill px-4" style="color: #64748b; font-weight: 500; text-transform: none;">
                        <i class="fas fa-briefcase mr-1"></i> License Info
                    </a>
                    <a href="{{ route('license-ownership.edit', $organization->id) }}" class="btn btn-material btn-material-primary rounded-pill px-4" style="text-transform: none; font-weight: 500;">
                ...
                <div class="cioas-panel">
                    ...
                    <div class="cioas-panel-body bg-light p-4">
                        ...
                        {{-- ================= OWNERSHIP SECTION ================= --}}
                        <div>
                            <form class="form-horizontal" id="organizationOwnershipForm" action="{{ route('license-ownership.store') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="license_id" value="{{ $organization->id }}">
                                @php
                                    $no_of_owners = $organization->no_of_owner ?? ($organization->no_of_dir ?? 1);
                                @endphp

                                @for ($i = 0; $i < $no_of_owners; $i++)
                                    @include('backend.pages.license.ownership.form', [
                                ...
                                @if ($organization->license_type_id == 2)
                                ...
                                <div class="cioas-actions mt-4 pt-3 border-top d-flex justify-content-end gap-3">
                                    <a href="{{ route('license.edit', $organization->id) }}" class="btn btn-material btn-secondary">
                                        Cancel
                                    </a>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#organizationOwnershipForm").on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('license-ownership.store') }}",
