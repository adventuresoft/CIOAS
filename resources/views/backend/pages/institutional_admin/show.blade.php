@extends('backend.master', ['mainMenu' => 'Institutional Admin', 'subMenu' =>'View'])
...
<div class="people-certificate-page">
    <div class="people-certificate-content">
        ...
        {{-- Header with Logos --}}
        @php
            $headerUnion = optional($user->addressInfo)->presentUnion ?? optional($user->institute)->union;
        @endphp
        ...
        <div class="citizen-title">
            <h4>ইনস্টিটিউশনাল অ্যাডমিন তথ্য বিবরণী</h4>
            <p>Institutional Admin Information Record</p>
        </div>

        {{-- Photo and ID block --}}
        <div class="photo-badge">
            <div class="photo-box">
                <img src="{{ $user->image ? asset($user->image) : asset('public/no-image-found.jpeg') }}" alt="Profile Photo">
            </div>
            <div class="id-info-columns">
                ...
                <div class="id-info-item"><span>Department :</span> {{ optional($user->department)->name ?? 'N/A' }}</div> </br>
                <div class="id-info-item"><span>Section :</span> {{ optional($user->section)->name ?? 'N/A' }}</div> </br>
                <div class="id-info-item"><span>Status :</span> {{ $user->status == 1 ? 'Active / Verified' : 'Pending Review' }}</div> </br>
            </div>
        </div>
        ...
        <div class="two-columns">
            <div class="col">
                ...
                <div class="info-row"><span class="info-label">Mobile No. :</span><span class="info-value">{{ $user->mobile ?? 'N/A' }}</span></div>
                <div class="info-row"><span class="info-label">NID No. :</span><span class="info-value">{{ $user->nid ?? 'N/A' }}</span></div>
                @if($user->people)
                <div class="info-row"><span class="info-label">Blood Group :</span><span class="info-value">{{ people_constant_option('blood_group')[optional($user->people)->blood_group ?? ''] ?? 'N/A' }}</span></div>
                <div class="info-row"><span class="info-label">Date of Birth :</span><span class="info-value">{{ optional($user->people)->date_of_birth ?? 'N/A' }}</span></div>
                @endif
            </div>
            <div class="col">
                <div class="info-row"><span class="info-label">Name (Bangla) :</span><span class="info-value">{{ optional($user->people)->bn_name ?? 'N/A' }}</span></div>
                <div class="info-row"><span class="info-label">Department :</span><span class="info-value">{{ optional($user->department)->name ?? 'N/A' }} @if(optional($user->department)->bn_name) ({{ optional($user->department)->bn_name }}) @endif</span></div>
                <div class="info-row"><span class="info-label">Section :</span><span class="info-value">{{ optional($user->section)->name ?? 'N/A' }} @if(optional($user->section)->bn_name) ({{ optional($user->section)->bn_name }}) @endif</span></div>
                <div class="info-row"><span class="info-label">Primary Role :</span><span class="info-value">{{ $user->roles->first()?->name ?? 'N/A' }}</span></div>
                @if($user->people)
                <div class="info-row"><span class="info-label">Gender :</span><span class="info-value">{{ people_constant_option('gender')[optional($user->people)->gender ?? ''] ?? 'N/A' }}</span></div>
                <div class="info-row"><span class="info-label">Religion :</span><span class="info-value">{{ optional($user->people)->religion->name ?? 'N/A' }}</span></div>
                @endif
            </div>
        </div>
        ...
        <div class="two-columns">
            ...
            <div class="col">
                <div class="info-row"><span class="info-label">Father's Name (Bangla) :</span><span class="info-value">{{ $user->familyInfo->father_name_bn ?? '' }}</span></div>
                ...
                <div class="info-row"><span class="info-label">Mother's Name (Bangla) :</span><span class="info-value"> {{ $user->familyInfo->mother_name_bn ?? '' }}</span></div>
        ...
        <div class="two-columns">
            <div class="col">
                ...
                <div class="info-row"><span class="info-label">District :</span><span class="info-value">{{ $user->addressInfo->permanentDistrict->name ?? optional(optional($user->addressInfo)->permanentVillage)->district->name ?? '' }}</span></div>
                <div class="info-row"><span class="info-label">Thana :</span><span class="info-value">{{ $user->addressInfo->permanentThana->name ?? optional(optional($user->addressInfo)->permanentVillage)->thana->name ?? '' }}</span></div>
                <div class="info-row"><span class="info-label">Union :</span><span class="info-value">{{ $user->addressInfo->permanentUnion->name ?? optional(optional($user->addressInfo)->permanentVillage)->union->name ?? '' }}</span></div>
                ...
                <div class="info-row"><span class="info-label">Village :</span><span class="info-value">{{ optional(optional($user->addressInfo)->permanentVillage)->en_name ?? '' }}</span></div>
                <div class="info-row"><span class="info-label">Ward :</span><span class="info-value">{{ optional(optional($user->addressInfo)->permanentWard)->en_ward_no ?? '' }}</span></div>
            </div>
            <div class="col">
                <h6 class="mb-2 font-weight-bold" style="color: #17a2b8;">বর্তমান ঠিকানা / Present Address</h6>
                <div class="info-row"><span class="info-label">District :</span><span class="info-value">{{ $user->addressInfo->presentDistrict->name ?? optional(optional($user->addressInfo)->presentVillage)->district->name ?? '' }}</span></div>
                <div class="info-row"><span class="info-label">Thana :</span><span class="info-value">{{ $user->addressInfo->presentThana->name ?? optional(optional($user->addressInfo)->presentVillage)->thana->name ?? '' }}</span></div>
                <div class="info-row"><span class="info-label">Union :</span><span class="info-value">{{ $user->addressInfo->presentUnion->name ?? optional(optional($user->addressInfo)->presentVillage)->union->name ?? '' }}</span></div>
                ...
                <div class="info-row"><span class="info-label">Village :</span><span class="info-value">{{ optional(optional($user->addressInfo)->presentVillage)->en_name ?? '' }}</span></div>
                <div class="info-row"><span class="info-label">Ward :</span><span class="info-value">{{ optional(optional($user->addressInfo)->presentWard)->en_ward_no ?? '' }}</span></div>
            </div>
        </div>
        @endif
        ...
        @if(!$loop->last)<hr style="border-top: 1px dotted #ccc;">@endif
        @endforeach
        @endif
        ...
        <div class="info-row"><span class="info-label">A/C No :</span><span class="info-value">{{ $fin->account_no ?? '' }} ({{ optional($fin->accountType)->en_name ?? '' }})</span></div>
        <div class="info-row"><span class="info-label">Bank :</span><span class="info-value">{{ optional($fin->bank)->en_name ?? '' }}</span></div>
...
<div class="no-print text-center my-4">
    <button class="btn btn-info px-5 py-2 btn-print-custom text-white" onclick="window.print()"><i class="fa fa-print"></i> Print / প্রিন্ট</button>
    <a href="{{ route('institutional-admin.edit', $user->id) }}" class="btn btn-warning px-5 py-2 ms-3 text-white"><i class="fa fa-edit"></i> Edit / এডিট</a>
    <a href="{{ route('institutional-admin.index') }}" class="btn btn-secondary px-5 py-2 ms-3"><i class="fa fa-arrow-left"></i> Back to List</a>
</div>
@endsection
