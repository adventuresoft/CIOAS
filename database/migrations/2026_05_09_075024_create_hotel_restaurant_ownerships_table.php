...
class CreateHotelRestaurantOwnershipsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('hotel_ownerships', function (Blueprint $table) {
            $table->id();

            $table->unsignedBigInteger('hotel_restaurant_id');
            ...
            $table->foreign('hotel_restaurant_id')
                ->references('id')
                ->on('hotel_restaurants')
                ->onDelete('cascade');
        });
    }
    /**
    ...
    {
        Schema::dropIfExists('hotel_restaurant_ownerships');
    }
}
